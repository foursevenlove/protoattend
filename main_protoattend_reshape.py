...
def main(unused_argv):
  ...
  def body(i, encoded_cand_keys_val, encoded_cand_queries_val,
    ...
    # 避免动态shape
    temp = tf.reshape(temp, [FLAGS.example_cand_size, 28, 28, 1])
  ...
  # 避免动态shape
  eval_cand_size_temp = int(
      math.floor(FLAGS.eval_cand_size / FLAGS.example_cand_size) * FLAGS.example_cand_size)
  encoded_cand_keys_val = tf.reshape(encoded_cand_keys_val, [eval_cand_size_temp, FLAGS.attention_dim])
  encoded_cand_queries_val = tf.reshape(encoded_cand_queries_val, [eval_cand_size_temp, FLAGS.attention_dim])
  encoded_cand_values_val = tf.reshape(encoded_cand_values_val, [eval_cand_size_temp, FLAGS.val_dim])
  ...
  def body2(j, encoded_val_keys, encoded_val_queries, encoded_val_values):
    ...
    # 避免动态shape
    temp = tf.reshape(temp, [eval_batch_size, 28, 28, 1])
    temp_keys, temp_queries, temp_values = model.cnn_encoder(
        temp, reuse=True, is_training=False)
  ...
  _, encoded_val_keys, encoded_val_queries, \
      encoded_val_values = tf.while_loop(
          ...
          shape_invariants=[
              ...
              tf.TensorShape([None, FLAGS.val_dim])])
  # 避免动态shape
  encoded_val_keys = tf.reshape(encoded_val_keys, [eval_batch_size, FLAGS.attention_dim])
  encoded_val_queries = tf.reshape(encoded_val_queries, [eval_batch_size, FLAGS.attention_dim])
  encoded_val_values = tf.reshape(encoded_val_values, [eval_batch_size, FLAGS.val_dim])
